...
import type { IStringResult } from "../../types/impl/helper";
...
export function findBestMatch(mainString: string, targetStrings: string[]): IStringResult {
...
export function findBestMatchArray(mainStrings: string[], targetStrings: string[]): IStringResult {
    const mainStringResults: IStringResult[] = [];
...
export function findBestMatch2DArray(mainStrings: string[], targetStrings: string[][]): IStringResult {
    let overallBestMatch: IStringResult = {
        ratings: [],
    ...
    mainStrings.forEach((mainString) => {
        targetStrings.forEach((targetArray, targetArrayIndex) => {
            ...
            targetArray.forEach((targetString) => {
                const currentRating = compareTwoStrings(cleanTitle(mainString.toLowerCase().trim()) ?? "", cleanTitle(targetString)?.toLowerCase().trim() ?? "");
