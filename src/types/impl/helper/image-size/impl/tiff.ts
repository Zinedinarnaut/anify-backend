...
import type { IImage, ISize } from "../";
import { readUInt, readUInt64, toHexString, toUTF8String } from "../../../../../helper/impl/image-size/impl/utils";
...
function readIFD(input: Uint8Array, filepath: string, { isBigEndian, isBigTiff }: TIFFFormat) {
...
function calculateBufferSize(filepath: string, ifdOffset: number): number {
    ...
    return ifdOffset + defaultSize > fileSize ? fileSize - ifdOffset - 10 : defaultSize;
}

function readFileBuffer(filepath: string, offset: number, size: number): Uint8Array {
...
function readTagValue(input: Uint8Array, type: number, offset: number, isBigEndian: boolean): number {
    switch (type) {
